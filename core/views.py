...
from django.utils.crypto import get_random_string # Importado para generación de contraseña
...
class CompanyViewSet(mixins.CreateModelMixin, 
    ...
    def subscribe(self, request, pk=None):
        ...
        # Lógica para setear restricciones
        if plan_name == 'BASICO':
...
def company_create_from_request_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        post_data = {
            ...
            'admin_rut': request.POST.get('admin_rut'), 
        ...
        # 🛑 SOLUCIÓN CLAVE: Verificar la unicidad del RUT de la Compañía antes de la creación
        ...
        try:
            with transaction.atomic():
                # 1. Crear Company (Tenant) - Solo se ejecuta si el RUT es único
                ...
                subscription.plan_name = client_request.plan_name
                subscription.active = True
                if client_request.plan_name == 'BASICO':
                    subscription.max_branches = 3
                elif client_request.plan_name == 'ESTANDAR':
                    subscription.max_branches = 6
                elif client_request.plan_name == 'PREMIUM':
                    subscription.max_branches = 9
                subscription.save()
                
                # 3. Crear el Admin Cliente (User)
                # FIX: Usar get_random_string para garantizar la generación de la contraseña
...
def company_list_template_view(request):
    ...
    # Renderizamos la plantilla core/company_list.html
...
def subscription_list_template_view(request):
    ...
    # Obtenemos las compañías con sus suscripciones para mostrar la tabla.
    companies = Company.objects.all().select_related('subscription').order_by('name')
